...
int main(void){
    ...
    uint8_t detected_addr = mpu_connect();
    
    int events = 0, last_event = 0;
    ...
    for(;;){
        ...
        if (mpu_read_accel_g(detected_addr, &r_ax, &r_ay, &r_az) == 0){
            ...
            char buffer[32];
            snprintf(buffer, sizeof(buffer), "Accel: ax=%dg ay=%dg\r\n", ax, ay);

            hc12_send_string(buffer);
            //printf("Accel: ax=%dg ay=%dg", ax, ay);
        } 
         
        int car_dist = hcsr04_read_cm()*100;
        
        detect_event(ax, ay, car_dist, &events, &last_event);
        char buffer[32];
        snprintf(buffer, sizeof(buffer), "distance: %d centimers\r\n", car_dist);
        hc12_send_string(buffer);
        //printf("distance: %d centimers\n", car_dist);
